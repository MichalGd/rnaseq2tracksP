...
make_ma <- function(df, cid, lfc_thr, label_suffix) {
  df <- df[df$baseMean > 0, ]
  df <- df[order(df$sig_tier), ]
  ggplot(df, aes(log10(baseMean), log2FoldChange, color=sig_tier)) +
    geom_point(alpha=0.5, size=0.8) +
    scale_color_manual(values=sig_colors, labels=sig_labels, name="Significance") +
...
for (i in seq_len(nrow(contrasts))) {
  ...
  res_shrunk <- tryCatch(
    ...
    error=function(e) {
      message("apeglm failed (", conditionMessage(e), "), trying ashr...")
      tryCatch(
  ...
  # Build data frames
  df_raw           <- as.data.frame(res_raw)
  df_raw$gene      <- rownames(df_raw)
  df_raw$sig_tier  <- classify_sig(df_raw$padj)
  ...
  sig <- df_shrunk[!is.na(df_shrunk$padj) & df_shrunk$padj < padj_thr &
...
if (gtf_path == "" || !file.exists(gtf_path)) {
  ...
  genes_ann <- data.table(
    ...
    chr=gtf_dt$chr, start=gtf_dt$start, end=gtf_dt$end, strand=gtf_dt$strand
  )
  ...
  for (i in seq_len(nrow(contrasts))) {
    ...
    padj_col <- paste0("padj_",cid); base_col <- paste0("baseMean_",cid)
    ...
    message(sprintf("[deseq2_de.R] Table: %s  (%d x %d)", outfile, nrow(tbl), ncol(tbl)))
  }
}
